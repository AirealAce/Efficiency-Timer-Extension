# Archived original native desktop app

This folder preserves the original C# Windows Forms version, its own shared engine, audio and tests. Build desktop/ReflectionTimer.Desktop/ReflectionTimer.Desktop.csproj here if needed. The current accessible app is in ../desktop and does not depend on this archive. Both versions use the same installed profile and instance lock; quit one before running the other. The original documentation follows for historical reference.
...
1. Open [the latest release](https://github.com/AirealAce/Reflection-Timer/releases/latest) and download **ReflectionTimer-3.12.9-win-x64.zip** under Assets. GitHub's **Code → Download ZIP** and **Source code** downloads are for developers.
