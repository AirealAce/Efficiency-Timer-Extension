...
The current repository is [AirealAce/Reflection-Timer](https://github.com/AirealAce/Reflection-Timer). The desktop update is 3.12.6; the old 3.6.4 build only had Ctrl+Alt+T. Downloading source or renaming a repository does not update an already installed app.
...
| Shortcut | Action |
| --- | --- |
| Ctrl+Alt+T | Show the main app. Preserve the selected tab; on Timer, select the first positive duration field. |
...
2. Extract the verified 3.12.6 Windows x64 release to a new folder. Do not overwrite files in a running app's folder.
...
4. Launch the installed app and check its executable's **Properties → Details → Product version**. It should say 3.12.6. Existing desktop shortcuts should point to the installed copy, not an old extracted download.
...
- Hide the main app in the tray; T should restore it. Minimize it and repeat.
...
The public packaging script runs this gate before producing a ZIP, alongside onboarding/install, delivery-safety, and receiver checks. It includes original synthesized tones and excludes local data, credentials, and all MP3 recordings.
