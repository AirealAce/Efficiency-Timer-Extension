...
The current repository is [AirealAce/Reflection-Timer](https://github.com/AirealAce/Reflection-Timer). The current accessible desktop source is 4.0.8. Local branch builds are not automatically published to Releases; the old 3.6.4 build only had Ctrl+Alt+T. Downloading source or renaming a repository does not update an already installed app.
...
2. Extract the verified 4.0.8 Windows x64 release to a new folder. Do not overwrite files in a running app's folder.
...
4. Launch the installed app and check its executable's **Properties → Details → Product version**. It should say 4.0.8. Existing desktop shortcuts should point to the installed copy, not an old extracted download.
