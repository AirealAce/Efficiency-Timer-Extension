...
internal sealed partial class PreviewWindow : Form
{
    ...
    internal PreviewWindow(PreviewApplication app, string view, Guid? prompt)
    {
        ...
        Text = view == "main" ? "Reflection Timer — App view · 4.0.2" : view == "compact" ? "Reflection Timer — Compact view · 4.0.2" : "Reflection Timer — Session end · 4.0.2";
        StartPosition = FormStartPosition.Manual; AutoScaleMode = AutoScaleMode.Dpi;
        Size = view == "main" ? new(940, 810) : view == "compact" ? new(320, 218) : new(560, app.Session.Engine.Snapshot.Prompts.Any(p=>p.Id==prompt&&p.EndedEarly)?525:440);
    ...
    private async void Receive(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        try {
            ...
            if (action == "compactSize") {
                ...
                Text="Reflection Timer — "+(IsTimeOnly?"Time-only":"Compact")+" view · 4.0.2";
