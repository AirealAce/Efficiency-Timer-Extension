# Reflection Timer Desktop 3.6.1
...
- **Settings → Audio** configures four events: **Success** (Level Up), **Failure** (Out of Health), **Low on time** (Battle (Trainer)), and **Session end** (the unchanged original `popup.mp3`). Your existing custom session-end MP3 is retained on upgrade. Each event can select from the local library, use any validated custom MP3 under 50 MB, restore its default, or preview it. Sound/mode selections save immediately. Previews and operation-result messages use the Timer volume; low-time and completion sounds use that session's volume. Previews never contact Sheets. Success feedback accompanies confirmed saves and uploads, not intermediate progress or draft autosaves; audio playback errors show a warning without recursively sounding a failure.
