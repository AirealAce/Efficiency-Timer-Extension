...
export function mountAudio({send,run}) {
  ...
  for(const kind of [1,2,3,0]){
    ...
    form.addEventListener('change',event=>{editor.dirty=true;editor.revision++;field('sound-fade-seconds').disabled=!field('sound-fade').checked;run(async()=>{await saveSound();if(event.target===field('sound-track'))await send('previewSound',{kind,quiet:true});});});
