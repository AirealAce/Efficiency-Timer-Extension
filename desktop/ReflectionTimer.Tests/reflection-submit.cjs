...
module.exports=async function reflectionSubmit(context,initial,check){
  ...
  for(const kind of ['natural','early','check-in']){
    ...
    for(const target of targets){
      ...
      await focus(page,target);await page.keyboard.press('Control+Enter');await submitted(page);
      const sent=await messages(page),entry=sent.filter(m=>m.action==='queue');
      check(entry.length===1&&entry[0].data.id==='submit-test'&&entry[0].data.text==='Saved response'&&entry[0].data.reason==='Saved reason',`${kind} Ctrl+Enter sends the current response and reason from ${target}`);
      check(!sent.some(m=>['saveForLater','skip','navigateReflection','toggle','end','checkIn'].includes(m.action)),`${kind} Ctrl+Enter overrides the focused action without changing the session`);
      await page.close();
    }
  }
  ...
  check(retry.data.text==='Final keystrokes'&&retry.data.reason==='Final reason'&&sent.filter(m=>m.action==='draft').length===2,'Retry sends the latest durable fields without rewriting an unchanged draft');
  ...
  await guarded.keyboard.press('Control+Enter');
  ...
  await guarded.locator('#later').focus();await guarded.keyboard.press('Control+Enter');
  ...
  for(const [view,initialize] of [['main',true],['compact',true],['reflection',false]]){
    const other=await open('early',view,initialize);await other.keyboard.press('Control+Enter');
