module.exports=async function settingsSuccess(context,initial,settings,check){
  for(const shortcut of [false,true]){
    ...
    try{
      ...
      const save=()=>shortcut?page.keyboard.press('Control+Enter'):page.getByRole('button',{name:'Save settings',exact:true}).click();
      await save();await page.waitForFunction(()=>window.pendingConnection);
      check(await page.evaluate(()=>!window.previewMessages.some(m=>m.action==='settingsSaveComplete')),(shortcut?'Ctrl+Enter':'Save settings')+' waits for the final connection write before success audio');
      ...
      check(await page.evaluate(()=>window.previewMessages.filter(m=>m.action==='settingsSaveComplete').length===1),(shortcut?'Ctrl+Enter':'Save settings')+' requests the configured success sound exactly once after completion');
      ...
      check(await page.evaluate(()=>!window.previewMessages.some(m=>m.action==='settingsSaveComplete')),(shortcut?'Ctrl+Enter':'Save settings')+' does not play success if saving fails');
